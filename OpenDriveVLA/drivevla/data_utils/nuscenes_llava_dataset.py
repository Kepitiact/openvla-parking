...
class LLaVANuScenesDataset(NuScenesE2EDataset):
    ...
    def __init__(self,
        ...
        cached_path = os.environ.get('CACHED_DATA_PATH', 'data/nuscenes/cached_nuscenes_info.pkl')
        if os.path.exists(cached_path):
            self.cached_nuscenes_data = pickle.load(open(cached_path, 'rb'))
        else:
            self.cached_nuscenes_data = {}
            rank0_print(
                f"Warning: {cached_path} not found. Falling back to placeholder conversations for smoke tests."
            )
    ...
    def _reorder_data_dict(self, list_data_dict):
        ...
        if self.in_nuscenes_order:
            for i, data_dict in enumerate(list_data_dict):
                raw_id = data_dict.get('id', data_dict.get('qa_id', ''))
                sample_token = data_dict.get('sample_id') or raw_id.split('_')[0]
