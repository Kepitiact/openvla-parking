...
def planning_evaluation(pred_trajs_dict, subset=None, only_vehicle=True):
    ...
    if subset:
        eval_tokens = [t for t in subset if t in gt_trajs_dict]
    else:
        eval_tokens = list(gt_trajs_dict.keys())

    # Prediction keys can be either raw sample token or token + '_trajectory'.
    def _has_pred(token):
        return token in pred_trajs_dict or (token + '_trajectory') in pred_trajs_dict

    def _get_pred(token):
        if token in pred_trajs_dict:
            return pred_trajs_dict[token]
        return pred_trajs_dict[token + '_trajectory']

    # Mini/smoke runs may only predict a subset of GT tokens.
    eval_tokens = [t for t in eval_tokens if _has_pred(t)]
    if len(eval_tokens) == 0:
        raise ValueError('No overlapping tokens between GT and predictions for evaluation.')

    for index, token in enumerate(tqdm(eval_tokens)):
        ...
        output_trajs = torch.tensor(_get_pred(token))
    ...
    print("\n-------------------------------------------------------------------")
    print(f"Processed total {len(eval_tokens)} samples")
