...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        for i, gt_mask in enumerate(map_mask[:-1]):
            ys, xs = np.where(gt_mask)
            gt_bbox = ([int(min(xs)), int(min(ys)), int(max(xs)), int(max(ys))]
                       if len(xs) > 0 else [0, 0, 0, 0])
            gt_labels.append(i + self.map_num_classes)
            gt_bboxes.append(gt_bbox)
            gt_masks.append(gt_mask)
        gt_labels = torch.tensor(gt_labels)
        gt_bboxes = torch.tensor(np.stack(gt_bboxes)) if gt_bboxes else torch.zeros((0, 4))
        gt_masks = torch.stack(gt_masks) if gt_masks else torch.zeros((0, self.canvas_size[0], self.canvas_size[1]), dtype=torch.uint8)
