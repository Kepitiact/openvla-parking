...
import os as _os
# CARLA parking Stage-1: warm-start UniAD track+map+seg on CARLA lot data.
# Paths are env-parametrized so the config is portable (HAL vs local).
#   NUSC_DATA_ROOT   -> nuScenes-format DB root (v1.0-carla DB + maps live here)
#   CARLA_INFOS_TRAIN / CARLA_INFOS_VAL -> parking_infos_temporal.pkl (train/test split)
#   CARLA_LOT_GT     -> lot_map_gt_<map>.json (seg GT source, approach B)
#   UNIAD_WARMSTART  -> nuScenes UniAD stage1 checkpoint (.pth) to warm-start from
_REPO = _os.path.abspath(_os.path.join(_os.getcwd(), _os.pardir))
data_root = (_os.environ.get("NUSC_DATA_ROOT") or _os.path.join(_REPO, "data", "nuscenes")).rstrip("/") + "/"
info_root = "data/infos/"
file_client_args = dict(backend="disk")
_carla_proc = _os.path.join(_REPO, "data_carla", "processed")
# Held-out split (scripts/split_infos_train_val.py): train = 1308 eps, val = slots
# 19/24. Falls back to the combined pkl if the split files aren't present yet.
_combined = _os.path.join(_carla_proc, "parking_infos_temporal.pkl")
_split_train = _os.path.join(_carla_proc, "parking_infos_train.pkl")
_split_val = _os.path.join(_carla_proc, "parking_infos_val.pkl")
ann_file_train = _os.environ.get("CARLA_INFOS_TRAIN") or (_split_train if _os.path.exists(_split_train) else _combined)
ann_file_val   = _os.environ.get("CARLA_INFOS_VAL") or (_split_val if _os.path.exists(_split_val) else _combined)
ann_file_test  = ann_file_val
carla_lot_gt = _os.environ.get("CARLA_LOT_GT") or _os.path.join(
    _REPO, "data_carla", "processed", "lot_map_gt_Town04_Opt.json")
